...
Player2_Health=200;
Player1_Health=200;
remaining_health2=200;
remaining_health1=200;
...
r=.75; %r is the radius of the circle
...
if remaining_health2<=0
    comet(x,y);
    r=3; %r is the radius of the circle
ang=0:0.01:2*pi; %0.01 is the angle step
xp=r*4*cos(ang);
yp=.25*r*sin(ang);
    blast=fill(x_sol(end) + xp,yi(d) + yp,'r');
...
xp=r*4*cos(ang);
yp=.25*r*sin(ang);
    blast=fill(min(x_sol2) + xp,yi(d2) + yp,'r');
